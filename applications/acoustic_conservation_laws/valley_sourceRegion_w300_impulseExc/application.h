...
namespace ExaDG::Acoustics
{
  ...
  // {
  // public:
  //   InitialConditionPressure(double const fPeak, dealii::Point<dim> const &sourcePos)
  //       : dealii::Function<dim>(1, 0.0),
  ...
  //   value(dealii::Point<dim> const &p, unsigned int const) const final
  //   {
  //     double const r = (sourcePos - p).norm();
  //     double const d = 343.0 / (std::sqrt(2) * pi * fPeak);
  //     double const p0 = 1.0;
  //     return p0 * std::exp(-1.0 * r * r / (d * d));
  //   }

  // private:
  //   double const pi;
  //   double const fPeak;
  //   dealii::Point<dim> sourcePos;
  // };
  ...
  private:
    ...
    set_field_functions() final
    {
      ...
      // dealii::Point<dim> sourcePos = {150.0, 13.0, -150.0};
      // const double fPeak = 100.0;
      // this->field_functions->initial_solution_pressure.reset(
      //     new InitialConditionPressure<dim>(fPeak, sourcePos));
    ...
    std::string pointwise_output_points_filename = "/home/fs72754/fkraxb02/dealII_exaDG/exadg_apps/applications/acoustic_conservation_laws/valley_sourceRegion_w300/monitoring_points_w300.0.txt";
