...
namespace ExaDG::Acoustics
{
  ...
  public:
    ...
    value(dealii::Point<dim> const &p, unsigned int const i) const final
    {
      ...
      for (const auto &pml_info : pml_infos)
      {
        ...
        double temp = 0.0;
        if (distance > 0.0) // -1.0e-8
        {
          temp = pml_info.speed_of_sound / (pml_info.pml_thickness - distance) -
                 1.0 * pml_info.speed_of_sound / (pml_info.pml_thickness);
          // std::cout << "p: " << p << " | i: " << i << " | normal[i]: " << pml_info.normal[i] << " | temp: " << temp << std::endl;
          temp *= std::abs(pml_info.normal[i]);
        }
        result += temp; // sum contributions from all PMLs
      }
      // AssertThrow(result >= 0.0,
      //             dealii::ExcMessage("result should be non-negative, but is: " +
  ...
  private:
    ...
    set_field_functions() final
    {
      ...
      pml_info.point_on_plane = {0.5, 0.0, 0.0}; // PML right
      pml_info.normal = {1.0, 0.0, 0.0};
      pml_infos.emplace_back(pml_info);

      pml_info.point_on_plane = {-0.5, 1.0, 0.0}; // PML left
      pml_info.normal = {-1.0, 0.0, 0.0};
      pml_infos.emplace_back(pml_info);
    ...
    std::string pointwise_output_points_filename = "/home/fs72754/fkraxb02/dealII_exaDG/exadg_apps/applications/acoustic_conservation_laws/cube_pml_opposing/coords_pointwise.txt";
