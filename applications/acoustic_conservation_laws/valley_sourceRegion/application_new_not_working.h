...
namespace ExaDG::Acoustics
{
  ...
  public:
    ...
    add_parameters(dealii::ParameterHandler &prm) final
    {
      ...
      prm.enter_subsection("Application");
      {
        ...
        prm.add_parameter("WriteIntervalField",
                          write_interval_field,
                          "Write interval for field output.",
                          dealii::Patterns::Double());

        prm.add_parameter("WriteIntervalPointwise",
                          write_interval_pointwise,
                          "Write interval for pointwise output.",
                          dealii::Patterns::Double());
        ...
        prm.add_parameter("PointwiseOutputPointsFilename",
                          pointwise_output_points_filename,
                          "File with coordinates for pointwise output.",
                          dealii::Patterns::FileName(),
                          false);

        prm.add_parameter("OutputDataDegree",
                          output_degree,
                          "Degree of output data.",
                          dealii::Patterns::Integer());

        prm.add_parameter("PmlBlockId",
                          pml_id,
                          "Block ID of the PML.",
                          dealii::Patterns::Integer());
      }
  ...
  private:
    ...
    create_grid(Grid<dim> &grid, std::shared_ptr<dealii::Mapping<dim>> &mapping) final
    {
              ...
              std::vector<unsigned int> const & /* vector_local_refinements*/)
      {
        ...
        for (const auto &cell : tria.active_cell_iterators())
        {
          ...
          if (cell->material_id() == this->pml_id)
          {
    ...
    set_field_functions() final
    {
      ...
      pml_info.point_on_plane = {295.0, 140.0, -10.0}; // PML right
      pml_info.normal = {1.0, 0.0, 0.0};
      pml_infos.emplace_back(pml_info);

      pml_info.point_on_plane = {0.0, 140.0, -10.0}; // PML left
      pml_info.normal = {-1.0, 0.0, 0.0};
      pml_infos.emplace_back(pml_info);

      pml_info.point_on_plane = {10.0, 150.0, -10.0}; // PML top
      pml_info.normal = {0.0, 1.0, 0.0};
      pml_infos.emplace_back(pml_info);

      pml_info.point_on_plane = {10.0, 140.0, 0.0}; // PML front
      pml_info.normal = {0.0, 0.0, 1.0};
      pml_infos.emplace_back(pml_info);

      pml_info.point_on_plane = {10.0, 140.0, -300.0}; // PML back
      pml_info.normal = {0.0, 0.0, -1.0};
      pml_infos.emplace_back(pml_info);
    ...
    create_postprocessor() final
    {
      ...
      pp_data.output_data.time_control_data.trigger_interval = this->write_interval_field;
      ...
      pp_data.output_data.degree = this->output_degree;
      ...
      pp_data.pointwise_output_data.time_control_data.trigger_interval = this->write_interval_pointwise;
      ...
      // Punkte aus Datei einlesen, falls angegeben, sonst Standardpunkt
      if (!pointwise_output_points_filename.empty())
      {
        pp_data.pointwise_output_data.evaluation_points =
            read_points_from_file<dim>(pointwise_output_points_filename);
      }
      else
      {
    ...
    std::string pointwise_output_points_filename = "";
    double target_radius = 0.2;
    unsigned int pml_id = 0;

    double const start_time = 0.0;
    double end_time = 0.1;
    double write_interval_field = 0.01;
    double write_interval_pointwise = 0.01;
    unsigned int output_degree = 1;
  };
} // namespace ExaDG::Acoustics
