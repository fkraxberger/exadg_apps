...
namespace ExaDG::Acoustics
{
  ...
  private:
    ...
    set_field_functions() final
    {
      ...
      pml_info.pml_thickness = 1.0;

      pml_info.point_on_plane = {5.0, 0.0, 0.0}; // PML right
      pml_info.normal = {-1.0, 0.0, 0.0};
      pml_infos.emplace_back(pml_info);

      pml_info.point_on_plane = {-5.0, 0.0, 0.0}; // PML left
      pml_info.normal = {1.0, 0.0, 0.0};
      pml_infos.emplace_back(pml_info);

      pml_info.point_on_plane = {0.0, 5.0, 0.0}; // PML top
      pml_info.normal = {0.0, 1.0, 0.0};
      pml_infos.emplace_back(pml_info);

      pml_info.point_on_plane = {0.0, 0.0, 5.0}; // PML front
      pml_info.normal = {0.0, 0.0, 1.0};
      pml_infos.emplace_back(pml_info);

      pml_info.point_on_plane = {0.0, 0.0, 5.0}; // PML back
      pml_info.normal = {0.0, 0.0, 1.0};
      pml_infos.emplace_back(pml_info);
