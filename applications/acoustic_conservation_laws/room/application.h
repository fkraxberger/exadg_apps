...
namespace ExaDG::Acoustics
{
  ...
  public:
    ...
    add_parameters(dealii::ParameterHandler &prm) final
    {
      ...
      prm.enter_subsection("Application");
      {
        ...
        prm.add_parameter("RestartWriteIntervalWallTime",
                          restart_write_interval_wall_time,
                          "Interval for writing restart files (wall time in seconds).",
                          dealii::Patterns::Double());
      }
  ...
  private:
    ...
    set_parameters() final
    {
      ...
      this->param.restart_data.write_restart = true;
      ...
      this->param.restart_data.interval_wall_time = restart_write_interval_wall_time;
              ...
              std::vector<unsigned int> const & /* vector_local_refinements*/)
      {
        ...
        for (const auto &cell : tria.active_cell_iterators())
        {
          ...
          if (cell->material_id() != 1 &&
              cell->material_id() != 2 &&
              cell->material_id() != 3) // 4 is the ID of pml domain
          {
    ...
    set_boundary_descriptor() final
    {
      ...
      // 1 S_1 damping=1%
      this->boundary_descriptor->admittance_bc.insert(
          std::make_pair(1, std::make_shared<dealii::Functions::ConstantFunction<dim>>(0.005)));
      // 2 S_2 damping=1%
      this->boundary_descriptor->admittance_bc.insert(
          std::make_pair(2, std::make_shared<dealii::Functions::ConstantFunction<dim>>(0.005)));
      // 3 S_3 damping=1%
      this->boundary_descriptor->admittance_bc.insert(
          std::make_pair(3, std::make_shared<dealii::Functions::ConstantFunction<dim>>(0.005)));
      // 4 S_4 damping=1%
      this->boundary_descriptor->admittance_bc.insert(
          std::make_pair(4, std::make_shared<dealii::Functions::ConstantFunction<dim>>(0.005)));
      // 5 S_5 damping=1%
      this->boundary_descriptor->admittance_bc.insert(
          std::make_pair(5, std::make_shared<dealii::Functions::ConstantFunction<dim>>(0.005)));
      // 6 S_6 damping=1%
      this->boundary_descriptor->admittance_bc.insert(
          std::make_pair(6, std::make_shared<dealii::Functions::ConstantFunction<dim>>(0.005)));
    }
    ...
    set_field_functions() final
    {
      ...
      // pml_info.point_on_plane = {300.0, 140.0, -150.0}; // PML right
    ...
    create_postprocessor() final
    {
      ...
      pp_data.output_data.time_control_data.trigger_interval = 1.0/48.0;
      ...
      pp_data.pointwise_output_data.time_control_data.trigger_interval = 1.0/16.0e3;
    ...
    std::string pointwise_output_points_filename = "/home/fs72754/fkraxb02/dealII_exaDG/exadg_apps/applications/acoustic_conservation_laws/room/monitoring_points_room.txt";
    ...
    double restart_write_interval_wall_time = 1e5; // in seconds
  };
} // namespace ExaDG::Acoustics
